...
from app.crud.all_events import get_all_visible_events
...
@router.get("/all-events/{user_id}", response_model=dict)
def read_all_visible_events(user_id: int, request: Request, db: Session = Depends(get_db)):
    base_query = get_all_visible_events(user_id, db)
    filters = parse_filters(EventInstanceOverview, request.query_params)
    results = db.query(EventInstanceOverview).from_statement(base_query.statement).filter(filters).all()
